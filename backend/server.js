  ...
  nev.confirmTempUser(url, (err, user) => {
    if (user) {
      nev.sendConfirmationEmail(user.username, function(err, info) {
        if (err) {
          return res.render('index', { mailConfirmed: true,  textmailconfirmedModal: 'Erreur : le lien de confirmation est incorrect', glyphiconmailconfirmedModal: 'glyphicon-remove', colormailconfirmedModal: 'red', temporaryserviceclosed:''});
        }
